...
grep -q 'Actions' <<<"$users_frame"
grep -q 'Users' <<<"$users_frame"
if grep -q 'Expired retention' <<<"$users_frame"; then
  echo 'users page rendered expired retention action' >&2
  exit 1
fi
grep -q 'aiman' <<<"$users_frame"
grep -q 'active' <<<"$users_frame"
if grep -Eq 'online|offline' <<<"$users_frame"; then
  echo 'users page rendered online state' >&2
  exit 1
fi
...
bo_menu_activate
unset BLACKOUT_TUI_CONFIRM
grep -q '^user:remove-expired$' "$events_file"
grep -q 'Remove expired users' <<<"$BO_MENU_RESULT"
bo_menu_back
remove_expired_frame="$(bo_menu_render)"
if grep -Eq 'expired[[:space:]]+expired' <<<"$remove_expired_frame"; then
