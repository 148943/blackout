...
if bo_db_user_insert evil 00000000-0000-0000-0000-000000000002 evil@example '0);DROP TABLE users;--' active 100 200 2>/dev/null; then
  echo "malicious numeric level accepted" >&2
  exit 1
fi
if bo_db_user_insert evil 00000000-0000-0000-0000-000000000003 evil2@example 0 active now 200 2>/dev/null; then
  echo "noninteger created_at accepted" >&2
  exit 1
fi
if bo_db_user_update aiman '200;DROP TABLE users;' 2>/dev/null; then
  echo "malicious expiry accepted" >&2
  exit 1
fi
if sqlite3 "$BLACKOUT_DB" "pragma table_info(users);" | grep -q '|password|'; then
  echo "password column exists" >&2
  exit 1
fi
sqlite3 "$BLACKOUT_DB" "select name from sqlite_master where type='table' and name='users';" | grep -qx users
