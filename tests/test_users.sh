...
grep -qx 'vless://00000000-0000-0000-0000-000000000001@vpn.example:80?type=xhttp&security=none&path=/xhttp&host=vpn.example#aiman' "$BLACKOUT_ETC_DIR/default-links.out"
if grep -q '#aiman-' "$BLACKOUT_ETC_DIR/default-links.out"; then
  echo "default share link added suffix after username" >&2
  exit 1
fi
...
if (bo_user_cmd expired-retention 5) >/dev/null 2>&1; then
  echo "expired retention command succeeded" >&2
  exit 1
fi
if bo_user_auto_remove_expired 1 >/dev/null 2>&1; then
  echo "auto-remove accepted retention override" >&2
  exit 1
fi
...
bo_user_remove_expired | grep -qx auto_fresh
